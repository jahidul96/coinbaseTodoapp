...
import {getSingleUserInfo} from "../functions/FBDB";
...
export default function Rewards() {
	...
	const [award_coin, setAward_Coin] = useState([]);
	const [myInfo, setMyInfo] = useState();

	const userMe = auth?.currentUser.uid;

	const dailyCoinFunc = async (id) => {
		try {
			await updateDoc(doc(db, "DailyAwards", id), {
				taken: true,
			});

			await updateDoc(doc(db, "Users", myInfo.uid), {
				userCoin: myInfo.userCoin + 20,
			});
			setShow(true);
		} catch (error) {
	...
	useEffect(() => {
		const awardRef = collection(db, "DailyAwards");
		const q = query(awardRef, orderBy("createAt", "asc"));
		onSnapshot(q, (querySnapshot) => {
			let award_data = [];
			querySnapshot.forEach((doc) => {
				award_data.push({value: doc.data(), id: doc.id});
			});
			setAward_Coin(award_data);
		});

		getSingleUserInfo(setMyInfo);
	}, []);
			...
			<ScrollView>
				...
				<View style={styles.profileWrapper}>
					...
					<Text style={styles.totalCoin}>
						{myInfo ? myInfo?.userCoin : 0} Coins
					</Text>
				</View>
...
const DailyCoins = ({value, click, infoFunc}) => (
	<TouchableOpacity
		style={styles.dailyimgWrapper}
		onPress={value.value.taken ? infoFunc : () => click(value.id)}
	>
		...
		<Text style={[styles.dayText, value.value.taken && {color: GRAY}]}>
			{value.value.day}
		</Text>
	</TouchableOpacity>
);
